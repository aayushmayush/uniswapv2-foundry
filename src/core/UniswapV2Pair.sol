...
contract UniswapV2Pair is UniswapV2ERC20, IUniswapV2Pair {
    ...
    function _safeTransfer(address token, address to, uint256 value) private {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR, to, value)); //equivalent to calling IERC20(token).transfer(to,value)
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        uint256 _kLast = kLast; //read once to save gas
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint256 denominator = (rootK * 5) + rootKLast; //approx rootK but if growth is very large it gets around 20 percent
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        uint256 balance0 = IERC20(token0).balanceOf(address(this)); //address of smart contract is getting called becaue its the router that first add lp token in contract then burns using burn
        uint256 balance1 = IERC20(token1).balanceOf(address(this));
        uint256 liquidity = balanceOf[address(this)]; //number of lptoken
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        unchecked {
            uint256 balance0Adjusted = balance0 * 1000 - amount0In * 3; //frontend will ask for some extra amount if its not extra the call will fail
